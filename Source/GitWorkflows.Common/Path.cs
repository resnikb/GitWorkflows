...
namespace GitWorkflows.Common
{
    ...
    public class Path : IEquatable<Path>, IEquatable<string>, IComparable<Path>, IComparable<string>, IComparable
    {
        ...
        private static string GetNormalizedPath(string relativeOrAbsolute)
        {
            ...
            {
                if (component == ParentDirectoryReference)
                {
                    // For parent reference, we have four options:
                    //  1) This is the first item on the stack
                    //  2) The previous item is also a parent reference
                    //  3) The previous item is a drive reference (ends with ':')
                    //  4) The previous item is a regular string
                    //
                    // For the first two cases, we have no choice but to push the parent reference.
                    // If the previous item is a drive reference, we can ignore the parent reference, as there is no going back
                    // Otherwise, we can resolve the parent reference, by poping the previous item off the stack
                    if (stack.Count == 0 || stack.Peek() == ParentDirectoryReference)
                        stack.Push(component);
                    else if (stack.Peek().Last() != ':')
                        stack.Pop();
                }
                else if (!string.IsNullOrEmpty(component) && component != CurrentDirectoryReference)
                    stack.Push(component);
            }
            ...
            // If we got empty path in the end, it is a relative path which refers to the current directory
            return path.Length > 0 ? path : CurrentDirectoryReference;
        }
